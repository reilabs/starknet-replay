...
pub enum Error {
    ...
    /// `crate::profiler::collect_profiling_info` when the index doesn't exist
    /// in the list of Sierra statements.
    ...
    /// `crate::profiler::collect_profiling_info` when the list of Sierra
    /// statements is empty.
    #[error("The list of Sierra statements is empty. Can't continue profiling.")]
    EmptyStatementList,

    /// `EmptyProgramCounterList` is returned in function
    /// `crate::profiler::collect_profiling_info` when the list of program
    /// counters is empty.
