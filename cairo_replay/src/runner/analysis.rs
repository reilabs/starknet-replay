...
mod tests {
    ...
    fn test_get_sierra_program_from_class_definition() {
        ...
        let sierra_program_json: serde_json::Value =
            serde_json::from_str(&sierra_program_json).unwrap_or_else(|_| panic!("Unable to parse {sierra_program_json_file} to json"));
        let contract_class: SierraContractClass = serde_json::from_value::<
            SierraContractClass,
        >(sierra_program_json)
        .unwrap_or_else(|_| panic!("Unable to parse {sierra_program_json_file} to SierraContractClass"));
        let sierra_program = get_sierra_program_from_class_definition(
            &contract_class,
        )
        .unwrap_or_else(|_| panic!("Unable to create Program {sierra_program_json_file} to \
             SierraContractClass"));
        ...
        let sierra_program_test_json: serde_json::Value =
            serde_json::from_str(&sierra_program_test_json).unwrap_or_else(|_| panic!("Unable to parse {sierra_program_test_file} to json"));
